...
public class VideoFavouriteFragment extends Fragment implements OnStartDragListener, OnPlaylistItemListChangedListener {
    ...
    private void ClickItem(View view, int position) {
        VideoPlayerActivity.launchWithUri(requireContext(), GetPlaybackUriUtils.forFavorites(position));
    }
    ...
    private void PlayAllItem(){
        VideoPlayerActivity.launchWithUri(requireContext(), GetPlaybackUriUtils.forFavorites(0));
    }

    private void DeleteAllItem(){
        MediaQueueDeleteDialog dialog = new MediaQueueDeleteDialog();
        dialog.setType(PlaylistConstants.TYPE_VIDEO_FAVOURITE);
        dialog.show(getParentFragmentManager(), PlaylistConstants.TAG_DELETE_DIALOG_QUEUE);
    }
